...
The bulletin archive implementation is retained but temporarily hidden on the website. It groups the Digital and Printout PDFs in `bulletins/manifest.json` by date. The newsletter archive reads `newsletters/manifest.json`, shows the two newest newsletters initially, and provides a control to reveal the complete list. The production build copies both archive directories into `dist`, so each automated commit to `main` publishes the updated manifests and PDFs through the existing GitHub Pages workflow.
